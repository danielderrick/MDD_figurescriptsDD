...
# module10_pathways_selected <- pathways_10 %>%
#   mutate(geneRatio = number_entities_found/number_entities_total) %>%
...
#  drop_na() %>%
  slice_min(order_by = c(fdr), n = 5, with_ties = FALSE) %>%
  arrange(superpathway, desc(geneRatio))
pathways_10_selected$name <- factor(pathways_10_selected$name, ordered = TRUE)
...
pdf("../multiomics_analysis/plots/MDD_F9d.pdf",
      # width = 3.1,
      # height =2.5,
      useDingbats = FALSE,)
  print(p)
...
p <- ggplot(pathways_10_selected, aes(x = geneRatio, y = str_wrap(name, 25),  size = found, color = fdr)) +
  geom_point() +
  scale_color_gradient(low = "red", high = "blue") +
...
p
  ...
  pdf("../multiomics_analysis/plots/MDD_F9e.pdf",
      width = 7,
      height =9,
      useDingbats = FALSE,)
  print(p)
