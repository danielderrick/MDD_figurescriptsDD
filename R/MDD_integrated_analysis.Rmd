...
if(file.exists(paste0(data_path,"MDD_multiomics_clusters.csv"))){
  ...
  #compare consensus clustering to previous single seed
}
...
multiomics_14_partition <- multiomics_14_features_data_ann %>%
   arrange(feature_type) %>%
  pull(Cluster) %>%
  set_names(multiomics_14_features_data_ann$feature_type) %>%
  as.cl_partition()
...
single_to_consensus_cors <- cor(t(multiomics_14_features_data_mean_dm), t(multiomics_14_single_features_data_mean_dm), method = "pearson")
...
#calculate the agreement of the consensus clustering to the single clustering
