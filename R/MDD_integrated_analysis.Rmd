...
multiomics_14_clusters <-c("1", "2", "3+14", "4", "5","6", "7+17", "8", "9+15", "10+12", "11","13","16", "18")
...
if(file.exists(paste0(data_path,"MDD_multiomics_clusters.csv"))){
  ...
  con_cons <- clue::cl_ensemble(list = lapply(1:25, function(x){
    load(paste0("../Data/MDD_multiomics_clstrngs_",nbr_clusterings,"_nrb_cnsns_",nbr_consensus,"_run_",x,".rda"))
  ...
  set.seed(100)
  ce_cons <- clue::cl_consensus(con_cons, control = list(nruns = 5))
  ...
  #Calculate the agreement between ensemble elements and the consensus clusetering
  ags_con <- sapply(con_cons, clue::cl_agreement, y = ce_cons, method = "FM")
  
  
  p <- ggplot(tibble(agreements = unlist(agrmnts)), aes(x = agreements)) +
    geom_density() +
    theme_bw()
  p
  ...
  p <- ggplot(tibble(agreement_to_consensus = ags_con, method = "FM"), aes(x = agreement_to_consensus)) +
    geom_density() +
    theme_bw()
  p
}
...
multiomics_14_features_data_ann <- multiomics_data_ann %>%
    mutate(Cluster = as.character(Cluster),
      Cluster = case_when(Cluster %in% c("3","14") ~"3+14",
                          Cluster %in% c("7","17") ~"7+17",
                          Cluster %in% c("9", "15") ~"9+15",
                          Cluster %in% c("10","12") ~"10+12",
                               TRUE ~Cluster),
...
#Annotate callouts with abbreviations of the type instead of changing the color
...
hm_multiomics_14_callouts_type <- Heatmap(multiomics_14_features_dm,
                       name = "lfc rr",
                       column_title = "multiomics_14 features",
                       column_title_gp = gpar(fontsize = 6),
                       width = ncol(multiomics_14_features_dm)*unit(4, "mm"),
