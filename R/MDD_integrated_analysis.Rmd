...
multiomics_14_clusters <-c("1+8", "2", "3+5", "4+17", "6+14", "7", "9", "10", "11", "12","13", "15","16", "18")
                          
cluster14_cols <- c(structure(RColorBrewer::brewer.pal(12, "Paired")[1:12], names = multiomics_14_clusters[1:12]), c("16" = "#222222", "18" = "#555555"))
...
multiomics_data_dm <- multiomics_data_dm[!is.nan(multiomics_data_dm[,1]),]
  ...
  if(file.exists(paste0(data_path,"MDD_multiomics_clusters.csv"))){
    ...
    #TODO replace this with a consensus clustering
    ...
    create_consensus_clustering <- function(mat = multiomics_data_dm,
      ...
      cons_list <- lapply(seq_len(nbr_consensus), function(c) {
        cons <- cl_ensemble(list = lapply(seq_len(nbr_clusterings), function(b) { #Create clusterings
          medoids <- sample(1:nrow(mat), size = clust_num, replace = FALSE)
          clusters  <- pam(x = mat, 
                           k = clust_num, 
                           medoids = medoids, 
                           pamonce = 5)})) %>%
...
clusters <- cl_class_ids(cons)
names(clusters) <- rownames(cons$.Data)

# cons_2 <- create_consensus_clustering(nbr_clusterings = nbr_clusterings)
# cons_3 <- create_consensus_clustering(nbr_clusterings = nbr_clusterings)
# 
# cons_cons <- cl_consensus(cl_ensemble(list(cons_1, cons_2, cons_3)), method = "HE", control = list(nruns = 5))
# cl_agreement(cons_1, cons_2,method = "Rand")
# cl_agreement(cons_1, cons_2,method = "NMI")
# cl_agreement(cons_1, cons_2,method = "J")
# cl_agreement(cons_1, cons_3,method = "Rand")
# cl_agreement(cons_1, cons_3,method = "NMI")
# cl_agreement(cons_1, cons_3,method = "J")

               
    write_csv(tibble(Cluster = clusters, feature_type = names(clusters)), file = paste0(data_path,"MDD_multiomics_run7_clusters.csv"))
  }

######temporary comparison of two final consensus clusterings
    run1_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run1_clusters.csv")) 
    run2_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run2_clusters.csv")) 
    run3_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run3_clusters.csv")) 
    run4_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run4_clusters.csv")) 
    run5_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run5_clusters.csv")) 
    run6_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run6_clusters.csv")) 
    run7_cluster_df <- read_csv(paste0(data_path,"MDD_multiomics_run7_clusters.csv")) 


    cl_table <- table(run6_cluster_df$Cluster, run7_cluster_df$Cluster)
    cl_table
    caret::confusionMatrix(cl_table)
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_module_module_correlations_reduced.pdf",
      width = 9, height = 8)
  res <- print(cor_hm)
  res <- dev.off()
}
...
multiomics_14_features_data_ann <- multiomics_data_ann %>%
    mutate(Cluster = as.character(Cluster),
      Cluster = case_when(Cluster %in% c("1","8") ~"1+8",
                          Cluster %in% c("4","17") ~"4+17",
                          Cluster %in% c("3", "5") ~"3+5",
                          Cluster %in% c("6","14") ~"6+14",
                               TRUE ~Cluster),
...
multiomics_14_features_data <- multiomics_14_features_data_ann %>%
  select(-Cluster)

#create a numeric matrix with feature row names
multiomics_14_features_dm <- multiomics_14_features_data %>%
  select(all_of(condition_order)) %>%
  as.matrix() 
rownames(multiomics_14_features_dm) <- multiomics_14_features_data$feature_type
multiomics_14_features_dm <- multiomics_14_features_dm[!is.nan(multiomics_14_features_dm[,1]),]

multiomics_14_features_ann <- multiomics_14_features_data_ann %>%
  select(Cluster, Type, Set) %>%
  as.data.frame()

#Create a matrix and heatmap of the mean combined18 values
multiomics_14_features_data_mean <- multiomics_14_features_data_ann %>%
  group_by(Cluster) %>%
  summarise(across(.cols = matches("_24|48"), .fns = mean, .groups = "drop"))

multiomics_14_features_count <- multiomics_14_features_data_ann %>%
  group_by(Cluster) %>%
  dplyr::count()

multiomics_14_features_data_mean_dm <- multiomics_14_features_data_mean %>%
  select(-Cluster) %>%
  as.matrix()
rownames(multiomics_14_features_data_mean_dm) <-multiomics_14_features_data_mean$Cluster

#Make heatmap with annotations
haRow_multiomics_14 <- HeatmapAnnotation(df = multiomics_14_features_ann,
                                    which = "row",
                                    col = list(Cluster = factor(cluster14_cols, levels = cluster14_cols, ordered = TRUE),
                                               Type = assay_cols,
                                               Set = c("shared" = "white", ligand_cols)))

hm_multiomics_14 <- Heatmap(multiomics_14_features_dm,
                       name = "lfc rr",
...
###multiomics_14 heatmap with ligand column annotations, RPPA and motif callouts
  callouts <- read_csv(paste0(data_path, "MDD_multiomics_heatmap_callouts.csv")) %>%
    filter(selected == "x")

  #Make heatmap with annotations
col_annotations <- tibble(condition = colnames(multiomics_14_features_dm)) %>%
  mutate(ligand = str_remove(condition, "_.*")) %>%
  select(ligand) %>%
  as.data.frame()

haCol_multiomics_14 <- HeatmapAnnotation(df = col_annotations,
                                    which = "column",
...
df <- multiomics_14_features_data_ann %>%
  #filter(!Type == "RNAseq") %>%
...
hm_multiomics_14_callouts <- Heatmap(multiomics_14_features_dm,
                       name = "lfc rr",
                       column_title = "multiomics_14 features",
                       column_title_gp = gpar(fontsize = 6),
                       width = ncol(multiomics_14_features_dm)*unit(4, "mm"),
                       col = colorRamp2(c(-1.5, 0, 1.5), c("#2166AC", "white", "#B2182B")),
                       cluster_rows = FALSE,
                       cluster_row_slices = FALSE, 
                       cluster_columns = FALSE,
...
hm_multiomics_14_mean <- Heatmap(multiomics_14_features_data_mean_dm,
                              name = "lfc ",
...
hm_multiomics_14_mean_obj <- Heatmap(multiomics_14_features_data_mean_dm,
                            name = "lfc ",
                            column_title = "multiomics_14 cluster means",
                            column_title_gp = gpar(fontsize = 12),
                            col = colorRamp2(c(-2, 0, 2), c("#2166AC", "white", "#B2182B")),
        ...
        if(abs(multiomics_14_features_data_mean_dm[i, j]) > mean_highlight_thresh)
            grid.text(sprintf("%.1f", multiomics_14_features_data_mean_dm[i, j]), x, y, gp = gpar(fontsize = 12))},
                            use_raster = FALSE)
...
#Order the multiomics_14 heatmap by the clustering of the mean values
#reorder the levels in multiomics_14_features ann that are used in  row_split
# dm_ordered <- multiomics_14_features_data_ann %>%
#   mutate(Cluster = fct_relevel(Cluster, (rownames(multiomics_14_features_data_mean_dm)[multiomics_14_cluster_order])))
# 
# hm_multiomics_14_ordered <- Heatmap(multiomics_14_features_dm,
#                        name = "lfc rr",
#                        column_title = "multiomics_14 features",
#                        column_title_gp = gpar(fontsize = 12),
#                        col = colorRamp2(c(-1.5, 0, 1.5), c("#2166AC", "white", "#B2182B")),
...
#           #             left_annotation = haRow_multiomics_14,
#                        use_raster = FALSE)
# hm_multiomics_14_ordered
#####

#Remove the cluster assignments to keep the code similar to the 
#other dataset processing
#Remove RNAseq and label rows
multiomics_14_noRNA_features_data <- multiomics_14_features_data_ann %>%
  select(-Cluster) %>%
  filter(!Type == "RNAseq")

#create a numeric matrix with feature row names
multiomics_14_noRNA_features_dm <- multiomics_14_noRNA_features_data %>%
  select(all_of(condition_order)) %>%
  as.matrix() 
rownames(multiomics_14_noRNA_features_dm) <- multiomics_14_noRNA_features_data$feature_type
multiomics_14_noRNA_features_dm <- multiomics_14_noRNA_features_dm[!is.nan(multiomics_14_noRNA_features_dm[,1]),]

multiomics_14_noRNA_features_ann <- multiomics_14_features_data_ann %>%
  select(Cluster, Type, Set)  %>%
  filter(!Type == "RNAseq")%>%
  as.data.frame()
...
if(write_csvs){
  df <- multiomics_14_features_data_ann %>%
  filter(!Type == "RNAseq")
  res <- write_csv(df, "../tables/MDD_multiomics_14_noRNAseq.csv")
    res <- write_csv(multiomics_14_features_data_mean, "../tables/MDD_multiomics_14_mean.csv")
}
#Make heatmap with annotations
haRow_noRNA_multiomics_14 <- HeatmapAnnotation(df = multiomics_14_noRNA_features_ann,
                                    which = "row",
                                    col = list(Cluster = factor(cluster14_cols, levels = cluster14_cols, ordered = TRUE),
                                               Type = assay_cols,
                                               Set = c("shared" = "black", ligand_cols)))
#####
hm_multiomics_14_noRNA <- Heatmap(multiomics_14_noRNA_features_dm,
                       name = "lfc rr",
                       column_title = "multiomics_14 features, no RNAseq",
                       column_title_gp = gpar(fontsize = 12),
                       col = colorRamp2(c(-1.5, 0, 1.5), c("#2166AC", "white", "#B2182B")),
                       cluster_rows = FALSE,
                       cluster_row_slices = FALSE, 
                       cluster_columns = FALSE,
                       row_split = multiomics_14_noRNA_features_ann$Cluster,
                       row_gap = unit(2, "mm"),
                       row_title = " ",
                       show_row_names = TRUE,
                       row_names_gp = gpar(fontsize = 2.5),
                       column_names_gp = gpar(fontsize = 10),
                       left_annotation = haRow_noRNA_multiomics_14,
                       use_raster = FALSE)
hm_multiomics_14_noRNA
...
if(create_pdfs){
  ...
  draw(hm_multiomics_14)
  draw(hm_multiomics_14_callouts)
  draw(hm_multiomics_14_mean)
  draw(hm_multiomics_14_noRNA)
  res <- dev.off()
} 

if(create_pdfs){
  pdf("../plots/MDD_integrated_multiomics_14_mean_heatmap.pdf",height = 8, width = 5) 
  draw(hm_multiomics_14_mean, heatmap_legend_side = "left")
  res <- dev.off()
} 
...
module_features <- multiomics_data_ann %>%
...
multiomics_14_features <- multiomics_14_features_data_ann %>%
  arrange(Cluster, Type) %>%
  mutate(Cluster = paste0("module_", Cluster),
         Cluster = factor(Cluster, levels = unique(Cluster))) %>%
  split(.$Cluster) 

res <- write_xlsx(multiomics_14_features,
                  path = "../tables/MDD_int_rr_multiomics_14_tables.xlsx")

##### Randomized cluster assignments for null set analysis
module_features_null <- multiomics_data_ann %>%
    mutate(Cluster = Cluster[sample(nrow(multiomics_data_ann))]) %>%
  arrange(Cluster, Type) %>%
  mutate(Cluster = paste0("module_", Cluster),
         Cluster = factor(Cluster, levels = unique(Cluster))) %>%
  split(.$Cluster) 
...
dm <- multiomics_14_features_data_ann %>%
  dplyr::select(-feature, -Cluster, -Type, -feature_type, -Set) %>%
  as.matrix()
rownames(dm) <- multiomics_14_features_data_ann$feature_type
...
df_UMAP <- umap(dm, config = custom_settings, na.rm = TRUE)$layout %>%
  ...
  rename(UMAP_1 = X1,
         UMAP_2 = X2) %>%
  left_join(multiomics_14_features_data_ann, by = "feature_type") 
...
dm <- multiomics_14_features_data_ann %>%
  ...
  dplyr::select(-feature, -Cluster, -Type, -feature_type, -Set) %>%
  as.matrix()
rownames(dm) <- multiomics_14_features_data_ann %>%
    filter(Type == "RNAseq") %>%
  pull(feature_type)
...
df_cors_test <- df %>%
  ...
  left_join(multiomics_14_features_data_ann, by = c("GCP_feature" = "feature")) %>%
...
# multiomics_14_features_data_mean_df <- multiomics_14_features_data_ann %>%
#   group_by(Cluster) %>%
#       summarise(across(.cols = matches("_24|48"), .fns = mean)) %>%
#     ungroup() %>%
#   mutate(Cluster = paste0("Module_",as.integer(Cluster)))
# multiomics_14_features_data_mean_dm <- multiomics_14_features_data_mean_df %>%
#   select(-Cluster) %>%
#   as.matrix()
# rownames(multiomics_14_features_data_mean_dm) <- multiomics_14_features_data_mean_df$Cluster
  
if_module_dm <- rbind(multiomics_14_features_data_mean_dm, if_dm) %>%
  t
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_IF_correlations.pdf")
  res <- print(cor_hm)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_IF_correlations_reduced.pdf",
      width = 9, height = 8)
  res <- print(cor_hm)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_module_phenotype_correlations_volcano.pdf",
      width = 6, height = 4)
  res <- print(p_volcano)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_module_phenotype_correlations_reduced.pdf",
      width = 9, height = 8)
  res <- print(cor_hm)
  res <- dev.off()
}
...
module_mean_data_ann <- multiomics_14_features_data_mean %>%
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_module_phenotype_scatterplots_signif.pdf",
      width =8, height = 2)
  res <- print(p_signif)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_module_phenotype_scatterplots.pdf",
      width =8, height = 14)
  res <- print(p)
  res <- dev.off()
}
...
if(create_pdfs) {
  pdf("../plots/MDD_multiomics_14_module_vs_phenotypes_scatterplots.pdf")
...
df <- multiomics_14_features_data %>%
  #drop_na() %>%
  select(feature, Type) %>%
  distinct()
...
p <- ggplot(df, aes(x = Type, fill = Type)) +
  ...
  coord_cartesian(ylim = c(0, 10000)) +
  scale_fill_manual(values = assay_cols)+
  labs(title = paste("Type distribution of the",length(unique(paste0(df$feature))), "features at unique timepoints in the multiomics_14-features MDD integrated dataset"),
       x ="Feature type") +
  theme_bw()+
  theme(axis.text.x =  element_text(angle = 90),
    axis.ticks = element_blank(),
...
p_log <- ggplot(df, aes(x = Type, fill = Type)) +
  ...
  scale_y_log10() +
  scale_fill_manual(values = assay_cols)+
  labs(title = paste("Type distribution of the",length(unique(paste0(df$feature))), "multiomics_14 features at unique timepoints in the MDD integrated dataset"),
       x ="Feature type") +
  theme_bw()+
  theme(axis.text.x =  element_text(angle = 90),
    axis.ticks = element_blank(),
...
p_log
if(create_pdfs){
  pdf("../plots/MDD_multiomics_14_feature_type_distribution.pdf")
  print(p)  
  res <- dev.off()
  pdf("../plots/MDD_multiomics_14_feature_type_distribution_log_scale.pdf")
  print(p_log)
  res <- dev.off()
} 

#bar chart of the multiomics_14 shared adn ligands
df <- multiomics_14_features_data_ann %>%
  count(Set, Cluster) %>%
...
feature_dist <- ggplot(df, aes(x = Set, y = n)) +
  geom_col() +
  scale_fill_manual(values = assay_cols)+
  labs(title = paste("Distribution of the",length(unique(paste0(multiomics_14_features_data_ann$feature))), "multiomics_14 features"),
       x ="Type") +
  theme_bw()+
  theme(axis.text.x =  element_text(angle = 90),
    axis.ticks = element_blank(),
...
p_hist <- ggplot(multiomics_14_features_data_ann, aes(x = as.factor(Cluster), fill = Type)) +
  geom_bar(position = "dodge") +  
  scale_fill_manual(values = assay_cols) +
...
df_int14 <- multiomics_14_features_data_ann %>%
 select(feature_type, Cluster) %>%
  filter(str_detect(feature_type, "RNAseq")) %>%
  rename(Integrated_cluster = Cluster) %>%
...
cluster_assignments <- multiomics_14_features_data_ann %>%
...
cycIF_multiomics_14_features_int <- multiomics_14_features_data_ann %>%
  filter(Type == "cycIF",
...
multiomics_14_features_with_cycIF_int <- c(multiomics_14_features_data_ann$feature_type, cycIF_multiomics_14_features_int$feature_type)
