...
#Create the heatmap
  hm <- Heatmap(matrix = df_as_matrix,
          ...
          na_col = "grey")
  pdf(paste0("../plots/MDD_standard_heatmap_",assay_name, ".pdf"))
  draw(hm)
  res <- dev.off()
  draw(hm)
}
...
custom_settings$n_neighbors = 25
  ...
  p <- ggplot(df_UMAP, aes(x = UMAP_1,
    ...
    scale_radius(breaks = c(0,1,4, 8, 24, 48), range = c(3, 8)) +
  ...
  pdf(paste0("../plots/MDD_standard_UMAP_",assay_name, ".pdf"), width = 6, height = 5)
  print(p)
  res <- dev.off()
  print(p)
}
...
plot_pca <- function(df, md, ligand_cols = ligand_cols, assay_name,
  ...
  p <- ggplot(df_pca, aes(x = PC1,
                          ...
                          colour = Ligand,
                          fill = Ligand)) +
    geom_point(shape = 21, alpha = .95) +
    scale_color_manual(values = ligand_EGF_cols) +
    scale_fill_manual(values = ligand_cols) +
    scale_radius(breaks = c(0,1,4, 8, 24, 48), range = c(4, 8)) +
    labs(title = paste("PCA embedding of ", assay_name, " data")) +
    xlab("PC1") +
    ylab("PC2") +
    guides(colour = guide_legend(override.aes = list(size = 4)),
           size = guide_legend(override.aes = list(shape = 19))) +
...
print(p)
pdf(paste0("../plots/MDD_standard_PCA_",assay_name, ".pdf"), width = 6, height = 5)
  print(p)
  print(p34)
  res <- dev.off()
}
...
md <- read_csv("../metadata/MDD_sample_annotations.csv", show_col_types = FALSE) %>%
...
plot_line_graphs <- function(df, md, ligand_cols, fc_thresh = 1, assay_name){
  ...
  panel.grid.major = element_line(colour = "gray95"))+
    facet_wrap(~str_wrap(feature, 9), scales = "free_y")
  p
